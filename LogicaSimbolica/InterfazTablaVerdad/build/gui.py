...
window.title("Super Interfaz de Tablas de Verdad")
...
generar_image = PhotoImage(
    file=relative_to_assets("Generar.png"))
generar= Button(
    image=generar_image,
    borderwidth=0,
    highlightthickness=0,
    command=lambda: tabla(),
    relief="flat"
)
generar.place(
    x=600.0,
...
def tabla():
    ...
    if len(entry) == 0:
        ...
        root.configure(background='#3E92CC') 
        ...
        operadores = {"∧":".","→": "->", "↔": "<->", "¬": "~", "v": "+"}
        xd=eliminar_parentesis(entry)
        func = reemplazar(xd, operadores)
        print(func)
        for i in func:
            if (i.lower()).isalpha() and (not(i.lower() in variables)):
                variables.append(i.upper())

        if len(variables) > 13:

            messagebox.showwarning(message="Ingrese sólo hasta cuatro variables.", title="Error")
            ...
            my_canvas.configure(yscrollcommand=my_scrollbar.set, bg='#3E92CC')
            my_canvas.bind('<Configure>', lambda e: my_canvas.configure(scrollregion=my_canvas.bbox("all")))

            second_frame = Frame(my_canvas, bg='#3E92CC')

            my_canvas.create_window((0, 0), window=second_frame, anchor="nw")

            text = '\t'

            lab = Label(second_frame, text=f"Función:   {transformacion(func).upper()}", bg='#3E92CC', fg='#000',
            ...
            lab = Label(second_frame, text=f"{text}", bg='#3E92CC', fg='#000', font=('Helvetica', 10, 'bold'))
            lab.pack(pady=5)

            for j in range(2 ** len(variables) - 1, -1, -1):
                binary = format(j, '0' + str(len(variables)) + 'b') 
                text = ''
                for char in binary:
                    text += dicc[char] + '\t'
                ...
                try:
                    ...
                    messagebox.showerror(
                        message="¡Error en la escritura de la función!",
                        title="Error De Sintaxis")
                    break
            texto_label_titulo_resultado = tk.Label(root, text="La proposicion es \n"+tipo_tabla(listica), fg='#000', bg='#3E92CC', font=('BreeSerif Regular', 16), wraplength=700)
            texto_label_titulo_resultado.place(x=330.0, y=520.0, anchor="nw")
...
def tipo_tabla(lista):
    if(all(elemento == "V" for elemento in lista)):
        return "Tautologia"
    elif(all(elemento == "F" for elemento in lista)):
